...
class IMU(Node):
    def __init__(self):
        ...
        self.did_ins_sub = rospy.Subscriber(
            "/ins_eul_uvw_ned", imu_msgs.DID_INS1, self.did_ins_callback
        )  # for DID_INS1
        self.pimu_sub = rospy.Subscriber(
            "/pimu", imu_msgs.PIMU, self.did_pimu_sub
        )  # for DID_PIMU
...
<<<<<<<< HEAD:mrobosub_hal/src/imu.py
        (x, y, z) = msg.theta
        self.did_ins_pub.publish((x, y, -z))
========
        self.did_ins_pub.publish(msg.theta)
>>>>>>>> comp:mrobosub_localization/src/imu.py
