...
class DVLPublisher:
    def __init__(self):
        ...
        self.pub_translational = rospy.Publisher(
            "dvl/translational_data", Dvl, queue_size=1
        )  # TODO edit DVL type to be translational
        # cone unit vectors
        self.coneA = np.array([np.cos(70 * np.pi / 180), 0, np.sin(70 * np.pi / 180)])
        self.coneB = np.array(
            [
                -np.cos(70 * np.pi / 180) * np.sin(30 * np.pi / 180),
                np.cos(70 * np.pi / 180) * np.cos(30 * np.pi / 180),
                np.sin(70 * np.pi / 180),
            ]
        )
        self.coneC = np.array(
    ...
    def run(self):
        ...
        while not rospy.is_shutdown():
            try:
                ...
                velocities = (
                    cone_A_vel * self.coneA
                    + cone_B_vel * self.coneB
                    + cone_C_vel * self.coneC
                )
                trans_response = Dvl()
                trans_response.velocityA = float(velocities[0])  # along surge axis
                trans_response.velocityB = float(velocities[1])  # along sway axis
                trans_response.velocityC = float(
                    velocities[2]
                )  # along heave axis (positive down)
