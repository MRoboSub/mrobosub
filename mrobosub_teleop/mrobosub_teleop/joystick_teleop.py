...
class DOF:
    ...
    def __init__(self, id : int, name : str, scale_t : float, scale_p : float, istoggleable : bool) -> None:
        ...
        self.twist_pub = self.create_publisher(Float64, f'/target_twist/{name}', qos_profile=1)
        self.pose_pub = self.create_publisher(Float64, f'/target_pose/{name}', qos_profile=1)
        self.pose_sub = self.create_subscriber(Float64, f'/pose/{name}', self.pose_callback)
...
class Joystick_teleop(Node):
    ...
    def __init__(self) -> None:
        ...
        self.input_subscriber = self.create_subscriber(Joy, "/joy", self.joystick_callback)
        self.wrench_pubs = [self.create_publisher(Float64, f'/output_wrench/{axis}', qos_profile=1) 
