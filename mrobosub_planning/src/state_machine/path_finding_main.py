...
class PathFollowing():
    ...
    def __init__(self):
        ...
        self.currentNode = Node(self.sub_pos)
    ...
    def setTargetYaw(self):
        deltax = self.goalNode.x - self.currentNode.x 
        deltay = self.goalNode.y - self.currentNode.y
        self.target_yaw = math.atan2(deltay, deltax)

    def setTargetSurge(self):
        deltax = self.goalNode.x - self.currentNode.x
        deltay = self.goalNode.y - self.currentNode.y
        self.target_surge = ((deltax * deltax) + (deltay * deltay))**(1/2)

    def arrivedAtGoal(self) -> bool:
        # TODO set the arrived at goal to be true if the current node is within a certain range of the goal node.
        return self.currentNode == self.goalNode

follower = PathFollowing()
follower.setCurrentNode()
...
path = finder.findPath(follower.currentNode, follower.goalNode, follower.occupancyGrid)
...
# REVIEW - do we need to add the target_pose_surge PID to the motor mixing or anything lke that.
PIO.set_target_twist_surge(follower.target_surge)



for node in path:
    follower.setGoalNode(node.x, node.y)
    follower.setCurrentNode()
    follower.setTargetYaw()
    follower.setTargetSurge()
    # comment yo
    break
