...
class ApproachBuoyOpen(TimedState):
    ...
    def handle_if_not_timedout(self) -> Outcome:
        ...
        if seen_glyph_outcome is not None: print(seen_glyph_outcome.glyph)
...
class Backup(TimedState):
    ...
    def initialize(self, prev_outcome):
        ...
        self.glyph_outcome = None

    def handle_if_not_timedout(self) -> Outcome:

        PIO.set_target_twist_surge(-0.15)
        PIO.set_target_pose_yaw(self.start_yaw)

        self.glyph_outcome = search_for_glyph(Gbl.preferred_glyph())

        return self.GlyphNotSeen()

    def handle_once_timedout(self) -> None:
        PIO.set_target_twist_surge(0)
        return self.glyph_outcome


class Pause(TimedState):
    TimedOut = Outcome.make('TimedOut')
    Pausing = Outcome.make('Pausing')
