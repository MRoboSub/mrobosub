...
class ApproachBinClosed(TimedState):
    ...
    surge_speed = 0.1  # max surge speed
    yaw_factor = 0.2
    centered_pixel_dist_thresh = 0.1  # threshold distance in pixels within which we say we have centered appropriatclass CenterToBinFromFar(TimedState):
...
class CenterLeftDropper(TimedState):
    ...
    def handle_if_not_timedout(self) -> Union[None, Reached]:
        ...
        if bin_camera_position and bin_camera_position.found:
            ...
            if y - self.centered_pixel_thresh / 2 > 0:
                surge_speed = self.surge_and_strafe_speed
            elif y + self.centered_pixel_thresh / 2 < 0:
                surge_speed = -1 * self.surge_and_strafe_speed
            if x - self.centered_pixel_thresh / 2 - self.dropper_offset / 2 > 0:
                sway_speed = self.surge_and_strafe_speed
            elif x + self.centered_pixel_thresh / 2 - self.dropper_offset / 2 < 0:
