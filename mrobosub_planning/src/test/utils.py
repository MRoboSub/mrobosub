...
class ObjectPositionServiceMock:
    def __init__(self) -> None:
        ...
        self.service = rospy.Service(
            "hsv_buoy_position", ObjectPosition, self._handle_obj_request
        )
...
class TargetReader:
    ...
    def __init__(self, *args: Any, **kwargs: Any) -> None:
        ...
        self._pose_heave_sub = rospy.Subscriber(
            "/target_pose/heave", Float64, partial(self._pose_callback, "heave")
        )
        self._pose_roll_sub = rospy.Subscriber(
            "/target_pose/roll", Float64, partial(self._pose_callback, "roll")
        )
        self._pose_yaw_sub = rospy.Subscriber(
            "/target_pose/yaw", Float64, partial(self._pose_callback, "yaw")
        )

        self._twist_heave_sub = rospy.Subscriber(
            "/target_twist/heave", Float64, partial(self._twist_callback, "heave")
        )
        self._twist_roll_sub = rospy.Subscriber(
            "/target_twist/roll", Float64, partial(self._twist_callback, "roll")
        )
        self._twist_yaw_sub = rospy.Subscriber(
            "/target_twist/yaw", Float64, partial(self._twist_callback, "yaw")
        )
        self._twist_surge_sub = rospy.Subscriber(
            "/target_twist/surge", Float64, partial(self._twist_callback, "surge")
        )
        self._twist_sway_sub = rospy.Subscriber(
            "/target_twist/sway", Float64, partial(self._twist_callback, "sway")
        )
        self._twist_pitch_sub = rospy.Subscriber(
            "/target_twist/pitch", Float64, partial(self._twist_callback, "pitch")
        )
