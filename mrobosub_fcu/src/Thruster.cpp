...
Wrench<double> Thruster::calculate_contribution(const Pose<double> &pose) {
    ...
    forces[2] = max_pos_thrust * sin(-pose.pitch());
...
int Thruster::thrust_to_pwm(double thrust, double voltage) {
    PWMFit* pwm_fit = &pwm_fits[3];
    // for(size_t i  = 0; i < pwm_fits.size(); ++i){
    //     if(pwm_fits[i].voltage == voltage){
    //         pwm_fit = &pwm_fits[i];
    //     }
    // }
...
double Thruster::pwm_to_thrust(int pwm, double voltage) {
    PWMFit* pwm_fit = &pwm_fits[3];
