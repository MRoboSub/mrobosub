...
class Node:
    ...
    def __init__(self):
        ...
        self.always_run = (
            rospy.myargv(sys.argv)[1] != "0"
        )  # input 1 for always_run to not have to do service calls always_run:=1

    def zed_callback(self, message):
        global img_seen_num, img_num
        recent_positions_local: List[Optional[ObjectPositionResponse]] = [None] * len(
            Targets
        )
        ...
        if should_run:
            ...
            for i, detection in enumerate(detections):
                ...
                object_position_response.x_position = (
                    bbox_area  # TODO: change this. for competition
                )
                ...
                if idx < 2:  # for red and blue gate symbols
                    if recent_positions_local[0] and recent_positions_local[0].found:
                        if (
                            recent_positions_local[0].x_theta
                            < object_position_response.x_theta
                        ):
                            if self.red_is_left:
                                idx = 1
                            else:
                        ...
                        if (
                            recent_positions_local[1].x_theta
                            < object_position_response.x_theta
                        ):
                            if self.red_is_left:
                                idx = 1
                ...
                if recent_positions_local[idx] and recent_positions_local[idx].found:
                    recent_positions_local[idx], recent_positions_local[1 - idx] = (
                        object_position_response,
                        recent_positions_local[idx],
                    )
