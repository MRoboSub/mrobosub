...
class AprilTagDetector:
    def __init__(self):
        ...
        self.april_tag_pub = rospy.Publisher(
            "/mrobosub_msgs/april_tag_detection", AprilTagDetector, queue_size=1
        )
        ...
        self.intrinsic_matrix = np.array(
            [449.0419, 0, 0], [0, 447.5708, 0], [350.6880, 237.6657, 1.0]
        )
    ...
    def handle_frame(self, data):
        ...
        if detections:
            for detect in detections:
                ...
                cv2.polylines(
                    cv_image, [corners], isClosed=True, color=(0, 255, 0), thickness=2
                )

                if detect.tag_id < 10:  # big tag
                    image_points = detect.corners.astype(np.float32)
                    retval, rvec, tvec = cv2.solvePnP(
                        self.object_points, image_points, flags=cv2.SOLVEPNP_IPPE_SQUARE
                    )

                if detect.tag_id >= 10:  # small tag at center
                    image_points = detect.corners.astype(np.float32)
                    retval, rvec, tvec = cv2.solvePnP(
                        self.small_object_points,
                        image_points,
                ...
                roll, pitch, yaw = self.calculate_euler_angles_from_rotation_matrix(
                    rotation_matrix
                )
                ...
                vertical_pos = (
                    40  # Adjust this value as needed for positioning text in the image
                )
                cv2.putText(
                    cv_image,
                    pos_text + orientation_text,
                    (10, vertical_pos),
