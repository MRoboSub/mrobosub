...
class PathmarkerPipeline:
    ...
    def process(self, source0):
        ...
        (self.blur_output) = self._blur(
            self._blur_input, self._blur_type, self._blur_radius
        )

        # Step HSV_Threshold0:
        self._hsv_threshold_input = self.blur_output
        (self.hsv_threshold_output) = self._hsv_threshold(
            self._hsv_threshold_input,
            self._hsv_threshold_hue,
            self._hsv_threshold_saturation,
            self._hsv_threshold_value,
        )
        # Step Find_Lines0:
        self._find_lines_input = self.hsv_threshold_output
        ...
        for line_i in self.find_lines_output:
            orientation_i = math.atan2(
                (line_i[0][0] - line_i[0][1]), (line_i[1][0] - line_i[1][1])
            )
        ...
        # Step Filter_Lines0:
        self._filter_lines_lines = self.find_lines_output
        (self.filter_lines_output) = self._filter_lines(
            self._filter_lines_lines,
            self._filter_lines_min_length,
            self._filter_lines_angle,
        )
    ...
    def _find_lines(input):
        ...
        if len(lines) != 0 and type(lines[0]) != type(None):
            for i in range(1, len(lines[0])):
                new_tmp = [
                    [int(lines[0][i, 0][0]), int(lines[0][i, 0][1])],
                    [int(lines[0][i, 0][2]), int(lines[0][i, 0][3])],
                ]
                tmp = PathmarkerPipeline.Line(
                    lines[0][i, 0][0],
                    lines[0][i, 0][1],
                    lines[0][i, 0][2],
                    lines[0][i, 0][3],
    ...
    def _filter_lines(inputs, min_length, angle):
        ...
        outputs = []
        for i in inputs:
            tmp = PathmarkerPipeline.Line(i[0][0], i[0][1], i[1][0], i[1][1])
            lines.append(tmp)
