...
class BinHsv(Node):
    ...
    def __init__(self):
        ...
        self.serv = TimedService(
            "/bin_object_position", ObjectPosition, self.timing_threshold
        )
    ...
    def handle_frame(self, msg):
        if self.serv.should_run() or self.always_run:
            ...
            self.enhanced_pub.publish(
                self.br.cv2_to_imgmsg(enhanced_img, encoding="bgr8")
            )
            self.annotated_pub.publish(
                self.br.cv2_to_imgmsg(annotated_img, encoding="bgr8")
            )

            response = ObjectPositionResponse()
            if detection is not None:
                x_theta, y_theta = utils.pixels_to_angles(
                    bgr_img, detection.x, detection.y
                )
                response.found = True
    ...
    def reconfigure_callback(self, config, level):
        self.hsv_params.update(
            {k: v for k, v in config.items() if k in self.hsv_params}
        )
        return config
