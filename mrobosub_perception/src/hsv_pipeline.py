...
class HsvPipeline:
    ...
    def filter_image(self, frame, return_enhanced=False):
        ...
        if self.gaussian_radius > 0:
            frame_hsv = cv2.GaussianBlur(
                frame_hsv,
                (self.gaussian_radius * 2 + 1, self.gaussian_radius * 2 + 1),
                0,
            )
        ...
        if self.erode_radius > 0:
            mask = cv2.erode(
                mask,
                np.ones((self.erode_radius, self.erode_radius), np.uint8),
                iterations=1,
            )
        if self.dilate_radius > 0:
            mask = cv2.dilate(
                mask,
                np.ones((self.dilate_radius, self.dilate_radius), np.uint8),
                iterations=1,
            )
    ...
    def find_rectangular_object(self, mask):
        ...
        if (selected_contour := self._get_rectangular_object(contours)) is not None:
            ...
            if len(points) == 4:
                ...
                rectCenter = calc_intersection_points(
                    leftSideCenter, rightSideCenter, topSideCenter, botSideCenter
                )
    ...
    def find_pathmarker_object(self, mask):
        ...
        if (selected_contour := self._get_pathmarker_object(contours)) is not None:
            ...
            if len(points) == 4:
                ...
                if length_between_point(
                    corner_topLeft, corner_topRight
                ) > length_between_point(corner_topLeft, corner_botLeft):
                    center_line = length_between_point(topSideCenter, rectCenter)
                    yaw_angle = np.degrees(
                        np.arcsin((topSideCenter[0] - rectCenter[0]) / center_line)
                    )
                else:
                    center_line = length_between_point(rightSideCenter, rectCenter)
                    yaw_angle = np.degrees(
                        np.arcsin((rightSideCenter[0] - rectCenter[0]) / center_line)
                    )
    ...
    def _simple_white_balance(self, mat, wb_shift, wb_scale):
        ...
        result[:, :, 1] = result[:, :, 1] - (
            (avg_a - wb_shift) * (result[:, :, 0] / 255.0) * (wb_scale / 100)
        )
        result[:, :, 2] = result[:, :, 2] - (
            (avg_b - wb_shift) * (result[:, :, 0] / 255.0) * (wb_scale / 100)
        )
    ...
    def _get_rectangular_object(self, contours):
        ...
        for contour in contours:
            ...
            if (
                ...
                and len(approx) >= 4
                and len(approx) <= 8
                and h > self.contour_min_height
                and w > self.contour_min_width
            ):
                maxHeight = h
                maxWidth = w
    ...
    def _get_circular_object(self, contours):
        ...
        for contour in contours:
            ...
            area_ratio = 1 - (
                abs(contour_area - cir_area) / max(contour_area, cir_area)
            )

            if (
                hw_ratio > maxHwRatio
                and area_ratio > maxAreaRatio
                and h > maxHeight
                and hw_ratio > self.height_width_ratio_thresh
                and area_ratio > self.area_container_perim_ratio_thresh
                and len(approx) >= 5
                and len(approx) <= 13
                and h > self.contour_min_height
                and w > self.contour_min_width
